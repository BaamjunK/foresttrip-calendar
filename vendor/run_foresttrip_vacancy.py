#!/usr/bin/env python3
# Vendored from https://github.com/NomaDamas/k-skill (foresttrip-vacancy, MIT License).
# 원본 대비 로컬 패치 2건 — "# [patch]" 주석으로 표시:
#   1) 로그인 시 시·도 드롭다운 순회에서 지점→시·도 매핑(sidos)을 세션에 보존
#      (실제 API 응답의 insttAreaNm 이 비어 있어 지역 정보는 이 방법뿐)
#   2) normalize_row 가 goodsId 를 보존 (가격 조회 API 가 goodsId 를 요구)
...
class Session:
    ...
    expires_at: float
    sidos: dict[str, str] = field(default_factory=dict)  # [patch] insttId -> 시·도명
...
def load_session_cache(path: Path) -> Session | None:
    ...
    try:
        ...
        return Session(
            ...
            sidos=dict(data.get("sidos", {})),  # [patch] 구버전 캐시 호환
        )
    except Exception:
        return None
    ...
    with sync_playwright() as p:
        ...
        for region in regions:
            ...
            for opt in options:
                ...
                if fid and name:
                    forests[fid] = name
                    sidos[fid] = str(region.get("text") or "").strip()  # [patch]
...
def normalize_row(
    row: dict[str, Any], forests: dict[str, str], sidos: dict[str, str] | None = None
) -> dict[str, Any]:
    ...
    return {
        ...
        "goods_id": row.get("goodsId") or "",  # [patch] 가격 조회 API 용
        "area": row.get("insttArea"),
        "capacity": row.get("mxmmAccptCnt"),
        "category": row.get("goodsClsscNm"),
        # [patch] insttAreaNm 은 실응답에서 항상 비어 있음 — 세션의 시·도 매핑 우선
        "region": (sidos or {}).get(instt_id) or row.get("insttAreaNm"),
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            for row in data:
                ...
                if use_dt < today or use_dt > last_day:
                    continue
                normalized = normalize_row(row, session.forests, session.sidos)  # [patch]
