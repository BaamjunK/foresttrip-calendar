...
- 링크 공유: "🔗 링크 복사"로 현재 조건(지역·주체·평점·정원·요금·이름·관심 목록)을
  담은 주소를 만든다. 링크로 열면 그 조건이 그대로 재현되지만 **내 설정을 덮어쓰지
  않는다** — 링크 조건은 저장하지 않고, 받은 관심 목록도 내 ★ 에 더하기만 한다.
  화면에서 무엇이든 만지는 순간부터 다시 내 설정으로 저장된다
- 전체 휴양림 목록: "휴양림 N곳" 타일을 누르면 전 지점(186곳)이 뜬다. 그 기간
  빈자리가 없어 달력에 안 나오는 곳도 여기서 찾고 관심 등록할 수 있다
- 예약 난이도: 조회 기간 중 빈자리가 있던 날 수를 세어 "만실"(0일) · "자리 귀함"
  (15% 이하) 배지를 붙인다. 목록·상세·오픈 목록에 함께 나온다. 평점이 좋아도
  자리가 안 나는 곳(수락산 동막골 ★4.8·만실)을 놓치지 않게 하는 용도다 —
  접근성 때문에 남는 곳도 있으니 품질의 증거는 아니다
- 기본 정원 상한: 처음 열면 "최대 6인" 이 걸려 있다. 대형 단체 객실이 목록을
  덮는 걸 막기 위한 것이고, 비우면 그 상태가 저장돼 다시 강제되지 않는다
- 기본 관심 휴양림: 좋다고 확인된 네 곳(영인산·갯골·광치·수락산 동막골)을 처음
  쓰는 브라우저에 시드로 넣는다. 한 번 심으면 사용자 목록이 진실 — 해제해도
  다시 넣지 않는다(`SEED_VERSION`)
- "닮음" 배지·필터: 위 네 곳의 공통점에서 뽑은 기준으로 비슷한 구성을 표시한다.
  전국 분포와 비교해 변별력이 있는 것만 남겼다 — **공립**, **숙박 객실의 독채
  비중 64% 이상**(전국 중간 51%), **보정 평점 4.1 이상**. 정원(4~6인)·면적
  (32~41㎡)·객실 수(18~32개)는 전국 중간값과 차이가 없어 기준에서 빼다.
  현재 35곳이 걸리고 네 곳 모두 포함된다. 네 곳에서 끌어낸 상관이지 "좋은 이유"
  는 아니므로 후보를 좁히는 용도로 본다
...
batch/fetch_profiles.py    전 지점 객실 구성 수집 (만실 지점 포함 — "닮음" 판별용)
...
- **추첨 병행 판정**: 정책 표의 선착순/추첨 칸은 실제와 어긋난다(추첨 칸에 값이
  ...
  정책 표에는 그 오픈이 어느 달 이용분인지 적혀 있지 않지만, 조회 API 의 날짜별
  상태(`rsrvtAvail`)로 확인할 수 있다. 아직 열리지 않은 날짜는 `OVER_DATE` 로
  온다 — 8/25 기준 강씨봉·축령산·칼봉산의 9월은 `Y`(예약 가능)인데 10월은 전부
  `OVER_DATE` 였다. 즉 **“매월 N일 오픈”은 그 다음 달 이용분을 연다**(9/1 → 10월분).
  국립은 “6주 전 수요일” 방식이라 월 단위가 아니고, 같은 9월 안에서도 날짜별로
  열림·미개방이 갈린다(유명산 9월말: `Y` 216건 / `OVER_DATE` 54건).
...
- **없는 데이터 — 설립·리모델링 연도**: 세 곳을 확인했지만 어디에도 없다.
  숲나들e 지점 페이지·시설정보 API(`selectFcFsRcfrsFcltInfo.do`)는 예약 관련
  필드만 주고 소개문에도 연도가 없다. 공공데이터포털
  [전국휴양림표준데이터](https://www.data.go.kr/data/15013111/standard.do)의 15개
  항목에도 개장일자가 없다(휴양림명·면적·수용인원·시설명·좌표 등). 국립자연휴양림
  [일반현황](https://huyang.forest.go.kr/kfsweb/kfi/kfs/cms/cmsView.do?mn=HUYG_01_03&cmsId=FC_000762)
  의 연도별 운영현황 표는 개소 수만 있고 "비고(신규개장)" 칸이 비어 있어 휴양림별
  개장연도를 알 수 없다. 리모델링 이력은 더더욱 공개되지 않는다.
  → 연식 필터는 만들 수 없다. 시설 노후는 평점에 상당 부분 드러나므로
  평점 하위 컷으로 대신한다.
