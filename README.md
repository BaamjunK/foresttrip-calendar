...
- 평점: 카카오맵 평점으로 "★4.3 이상" 같은 필터를 걸 수 있다(국립·공립만 수집).
  리뷰가 1~3건인 곳이 5.0 이나 1.0 으로 튀어 순위를 뒤집으므로, 표본이 적을수록
  전체 평균 쪽으로 당기는 보정값을 계산해 비교에 쓴다(표시는 원점수와 리뷰 수).
- 배편 필요 제외: 차로 못 들어가는 섬을 걸러낸다. 이름에 "~도" 가 들어가도
  연륙교가 있으면 차로 간다(석모대교·완도대교·진도대교·안면대교·무의대교·
  새만금방조제) — 실제로 배가 필요한 곳만 목록에 적어 둔다
...
batch/fetch_ratings.py     카카오맵 평점 수집 (국립·공립, 14일 지난 것만 하루 40곳)
